...
/**
 * Padrão Template Method: Subclasse concreta que implementa os passos 
 * para gerar o relatório específico de ocorrências.
 */
@Component
public class GeradorRelatorioOcorrencia extends GeradorRelatorioTemplate<Ocorrencia, RelatorioOcorrenciaDTO> {

    private final OcorrenciaRepositorio ocorrenciaRepositorio;
    private final AtendimentoRepositorio atendimentoRepositorio;

    public GeradorRelatorioOcorrencia(OcorrenciaRepositorio ocorrenciaRepositorio, AtendimentoRepositorio atendimentoRepositorio) {
        this.ocorrenciaRepositorio = ocorrenciaRepositorio;
        this.atendimentoRepositorio = atendimentoRepositorio;
    }

    @Override
    protected List<Ocorrencia> buscarDados() {
        return ocorrenciaRepositorio.findAll();
    }

    @Override
    protected RelatorioOcorrenciaDTO processarDado(Ocorrencia ocorrencia) {
